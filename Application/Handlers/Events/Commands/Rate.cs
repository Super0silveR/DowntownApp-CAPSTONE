...
namespace Application.Handlers.Events.Commands
{
    public class Rate
    {
        ...
        public class Command : IRequest<Result<Unit>?>
        {
            ...
            public EventRateCommandDto Rate { get; set; } = new EventRateCommandDto();
        }

        /// <summary>
        /// Handler class used to handle the rating of an Event.
        /// </summary>
        public class Handler : IRequestHandler<Command, Result<Unit>?>
        {
            ...
            public async Task<Result<Unit>?> Handle(Command request, CancellationToken cancellationToken)
            {
                ...
                _mapper.Map(request.Rate, @event);

                bool result = await _dataContext.SaveChangesAsync(cancellationToken) > 0;

                if (!result)
                    return Result<Unit>.Failure("Failed to rate an Event.");
        ...
        public class Validator : AbstractValidator<Command>
        {
            public Validator()
            {
                RuleFor(x => x.Rate).SetValidator(new EventRateCommandDtoValidator());
            }
        }
    }
}
